...
void setup() {
  ...
  if (psramFound()) {
    config.frame_size = FRAMESIZE_UXGA;  // Haute résolution si PSRAM disponible
    config.jpeg_quality = 10;
    config.fb_count = 2;  // Double buffer pour meilleure fluidité
  } else {
    config.frame_size = FRAMESIZE_VGA;  // VGA au lieu de SVGA pour éviter crash
    config.jpeg_quality = 12;
    config.fb_count = 1;
  }
  
  // Configuration supplémentaire pour stabilité
  ...
  s->set_framesize(s, FRAMESIZE_VGA);  // VGA pour bon équilibre qualité/performance
  
  // Optimisations du capteur
  ...
  s->set_awb_gain(s, 1);       // Gain auto balance des blancs
  ...
  s->set_colorbar(s, 0);       // 0 = Désactiver la barre de couleur de test
